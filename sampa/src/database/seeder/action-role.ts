...
const actionsArray: {
  ...
  },
  {
    ...
    actionNames: [
      ...
      {
        name: ActionEnum.ApprovedAssignSAM,
        process: ProcessEnum.AssessmentLayer,
      },
      {
        name: ActionEnum.SAMAssignedAssignSAM,
        process: ProcessEnum.AssessmentLayer,
      },
      {
        name: ActionEnum.SAMAssignedAddSupervise,
        process: ProcessEnum.AssessmentLayer,
      },
      {
  ...
  {
    roleName: ['pke SAM', 'soc SAM', 'csa SAM', 'sca SAM', 'ava SAM'],
    actionNames: [
      {
        name: ActionEnum.SAMAssignedAddSupervise,
        process: ProcessEnum.AssessmentLayer,
      ...
      {
        name: ActionEnum.LayerAssessmentReviewAcceptBySAM,
        process: ProcessEnum.AssessmentLayer,
      },
      {
        name: ActionEnum.LayerAssessmentReviewNeedModificationsBySAM,
        process: ProcessEnum.AssessmentLayer,
      },
      // Others
      {
...
export const ActionRoleSeeder = async (datasource: DataSource) => {
  for (let i = 0; i < actionsArray.length; i++) {
    ...
    if (typeof element.roleName === 'string') {
      ...
      for (let i = 0; i < element.roleName.length; i++) {
        const name = element.roleName[i];
        const roleRecord = await datasource
          .getRepository(Role)
