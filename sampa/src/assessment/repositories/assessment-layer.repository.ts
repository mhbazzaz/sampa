...
export class AssessmentLayerRepository extends AbstractRepository<AssessmentLayer> {
  ...
  //------------------------------
  async addSAM(
    data: UpdateAssessmentLayerAddSAMsDto,
    member: Member,
    memberRoles: Role[],
  ): Promise<boolean> {
    const queryRunner = this.dataSource.createQueryRunner();
    ...
    try {
      for (let i = 0; i < data.dataList.length; i++) {
        ...
        await this.assessmentTeamRepository.findAndDeleteTransactionable(
          {
            assessmentLayerId: element.layerId,
            memberId: Not(In(element.SAMIds)),
            isLead: true,
            isMember: false,
          },
          queryRunner,
        ...
        for (let i = 0; i < element.SAMIds.length; i++) {
          const supervisorId = element.SAMIds[i];
          const groupMembership =
            await this.assessmentTeamRepository.findOneTransactionable(
              {
                where: {
    ...
    const actionIds = actions
      .filter((a) => {
        if (
          ...
          a.name === ActionEnum.ApprovedAssignSAM
        ) {
          return false;
        }
  ...
  async getOneAssessmentLayerQueryBuilder(
    ...
    if (!hasReadAccess) {
      ...
      if (isInternal) {
        ...
        if (isSAM) {
          query.andWhere(
            '(assessmentTeams.memberId = :memberId AND assessmentTeams.isSAM = true)',
            { memberId: memberId },
          );
  ...
  async getUsersLayers(
    ...
    let queryBuilder = this.assessmentLayerRepository
      ...
      .leftJoinAndSelect('assessmentLayer.assessmentTeams', 'assessmentTeams')
      .leftJoinAndSelect('assessmentRequest.environment', 'environment')
      .leftJoinAndSelect('assessmentRequest.asset', 'asset');
