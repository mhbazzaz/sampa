...
export class AssessmentRequestService {
  ...
  async createAssessmentRequest(
    ...
    const applicantGroups = await this.groupMembershipRepository.findAll({
      where: {
        isMember: true,
        userId: member.id,
      },
    ...
    const applicantManager = await this.groupMembershipRepository.findOne({
      where: {
        ...
        group: {
          id: In(
            applicantGroups.map((applicantGroup) => applicantGroup.groupId),
          ),
          assetId: requestData.asset.id,
        },
      },
    });
    ...
    await this.flushRequestStateActionLog({
      ...
      currentStateId: null,
      nextStateId: createdRequest.stateId,
    });

    await this.flushManyLayerStateActionLogs({
      action: initialAction,
      member,
      memberRoles,
      assessmentRequestId: createdRequest.id,
      layers: createdLayers,
      nextLayerStateId: createdLayers[0]?.stateId ?? null,
    });
  ...
  async updateAssessmentRequestStatusByActionMyTeam(
    ...
    await this.flushRequestStateActionLog({
      ...
      currentStateId,
      nextStateId: stateTransition.id,
    });
  ...
  async updateAssessmentRequestStatusByAction(
    ...
    if (data.action === ActionEnum.ApprovedAccept) {
      ...
      await this.flushManyLayerStateActionLogs({
        ...
        assessmentRequestId: request.id,
        layers,
        nextLayerStateId: stateTransition.id,
      });
    }
  ...
  async getAssessmentReports(filters: AssessmentReportFilterDto) {
    ...
    const result =
      await this.assessmentRequestRepository.getAssessmentReports(filters);
    ...
    try {
      ...
      allEmployees.forEach((emp: any) => {
        ...
        if (emp.ADUserName) {
          const username = emp.ADUserName.split('\\').pop() || emp.ADUserName;
          employeeMap.set(username, emp);
          employeeMap.set(emp.ADUserName, emp);
        }
      });

      result.data.forEach((asset: any) => {
        asset.assessmentRequests?.forEach((req: any) => {
          const emp =
            employeeMap.get(req.applicantId) ||
            (req.applicant?.username &&
              employeeMap.get(req.applicant.username));

          if (emp) {
            req.departmentName =
              emp.DepartmentName || emp.departmentName || '-';
            req.groupName = emp.GroupName || emp.groupName || '-';
            req.managementName =
              emp.ManagementName || emp.managementName || '-';
          } else {
            req.departmentName = '-';
            req.groupName = '-';
            req.managementName = '-';
      ...
      console.log(
        'Failed to fetch employee details for report enrichment:',
        error.message,
      );
    }
  ...
  async exportAssessmentReports(
    filters: AssessmentReportFilterWithoutPaginateDto,
    format: ExportFormatEnum,
  ) {
    const exportFilters = { ...filters, skip: 0, take: 999999 };
    const { data } = await this.getAssessmentReports(exportFilters);
    ...
    const worksheet = workbook.addWorksheet('Assessment Reports');
    ...
    worksheet.columns = [
      {
        header: 'Hierarchy (Asset > Request > Layer)',
        key: 'hierarchy',
        width: 65,
      },
      { header: 'Reference ID / Req No.', key: 'refNum', width: 42 },
      { header: 'Type / Environment', key: 'typeEnv', width: 30 },
      {
        header: 'Asset Details (Dep / Mgt / Grp / Baseline)',
        key: 'assetDetails',
        width: 45,
      },
      { header: 'Status / State', key: 'status', width: 38 },
      { header: 'Created / Updated At', key: 'dates', width: 25 },
      { header: 'Items / Iterations', key: 'count', width: 18 },
      { header: 'Critical', key: 'crit', width: 14 },
      { header: 'High', key: 'high', width: 14 },
      { header: 'Medium', key: 'med', width: 14 },
      { header: 'Low', key: 'low', width: 14 },
    ];

    worksheet.columns.forEach((col) => {
      if (col) col.font = { name: 'Segoe UI', size: 10 };
    });
    ...
    headerRow.eachCell((cell, colNumber) => {
      cell.border = {
        ...
        right:
          colNumber < 11
            ? { style: 'thin', color: { argb: 'FF374151' } }
            : undefined,
      };
    });
    ...
    data.forEach((asset: any) => {
      const firstReq = asset.assessmentRequests?.[0] || {};
      const department = firstReq.departmentName || '-';
      const management = firstReq.managementName || '-';
      const groupName = firstReq.groupName || '-';

      const assetRow = worksheet.addRow({
        hierarchy: `🏢 ASSET: ${asset.title || 'Unknown'}`,
        refNum: asset.referenceId || '-',
        typeEnv: asset.assetType?.name || asset.assetType?.title || '-',
        assetDetails: `Dep: ${department}\nMgt: ${management}\nGrp: ${groupName}`,
        status: '-',
        dates: `C: ${formatDate(asset.createdAt)}\nU: ${formatDate(asset.updatedAt)}`,
        count: `${asset.assessmentRequests?.length || 0} Requests`,
        crit: asset.totalCriticalVulnerabilities || 0,
        high: asset.totalHighVulnerabilities || 0,
        med: asset.totalMediumVulnerabilities || 0,
        low: asset.totalLowVulnerabilities || 0,
      });

      assetRow.outlineLevel = 0;
      assetRow.height = 45;
      assetRow.font = {
        name: 'Segoe UI',
        bold: true,
      ...
      asset.assessmentRequests?.forEach((req: any) => {
        const reqRow = worksheet.addRow({
          hierarchy: `      📄 REQ: ${req.requestNumber || 'Unknown'}`,
          refNum: req.requestNumber || '-',
          typeEnv: req.environment?.name || '-',
          assetDetails: `Baseline: ${req.assetToAuditBaseline || '-'}\nDep: ${req.departmentName || '-'}\nMgt: ${req.managementName || '-'}`,
          status: req.state?.nameFa || req.state?.name || '-',
          dates: `C: ${formatDate(req.createdAt)}\nU: ${formatDate(req.updatedAt)}`,
          count: `${req.assessmentLayers?.length || 0} Layers`,
          crit: req.totalCriticalVulnerabilities || 0,
          high: req.totalHighVulnerabilities || 0,
          med: req.totalMediumVulnerabilities || 0,
          low: req.totalLowVulnerabilities || 0,
        });

        reqRow.outlineLevel = 1;
        reqRow.height = 45;
        reqRow.font = {
          name: 'Segoe UI',
          bold: true,
        ...
        req.assessmentLayers?.forEach((layer: any) => {
          const layerRow = worksheet.addRow({
            hierarchy: `            🔍 LAYER: ${layer.assessmentType?.name || 'Unknown'}`,
            refNum: '-',
            typeEnv: layer.assessmentType?.description || '-',
            assetDetails: '-',
            status: layer.state?.nameFa || layer.state?.name || '-',
            dates: `C: ${formatDate(layer.createdAt)}\nU: ${formatDate(layer.updatedAt)}`,
            count: `${layer.iterationCount || 0} Iters`,
            crit: layer.criticalVulnerabilitiesCount || 0,
            high: layer.highVulnerabilitiesCount || 0,
            med: layer.mediumVulnerabilitiesCount || 0,
            low: layer.lowVulnerabilitiesCount || 0,
          });

          layerRow.outlineLevel = 2;
          layerRow.height = 25;
          layerRow.font = {
            name: 'Segoe UI',
            size: 9,
    ...
    const foundRequest =
      await this.assessmentRequestRepository.getRequestWhetherUserCanReadItOrItIsUsers(
        ...
        [
          ...
          'requestSpecContents',
          'testcaseContents.testcaseRemediates',
          'testcaseContents.testcaseItem.testcaseGroup.assessmentType',
          'assessmentLayers.assessmentTeams.member',
          'assessmentLayers.assessmentType',
          'assessmentLayers.state',
        ],
  ...
  async cartable(query: GetCartableDto, memberRoles: Role[], member: Member) {
    const actions = await this.actionRepository.findAll({
      ...
      where: {
        roles: {
          id: In(memberRoles.map((memberRole) => memberRole.id)),
        },
      },
    });
  ...
  //------------------------------
  private async flushRequestStateActionLog(input: {
    action: ActionEnum;
    member: Member;
    memberRoles: Role[];
    assessmentRequestId: string;
    ...
    await this.actionLogBufferService.flushToActionLog(
      { assessmentRequestId: input.assessmentRequestId },
      {
        ...
        roleIds: input.memberRoles.map((role) => role.id),
        action: input.action,
        status: ActionLogStatusEnum.SUCCESS,
        assessmentRequestCurrentStateId: input.currentStateId ?? null,
        assessmentRequestNextStateId: input.nextStateId ?? null,
        assessmentLayerCurrentStateId: null,
  ...
  //------------------------------
  private async flushLayerStateActionLog(input: {
    action: ActionEnum;
    member: Member;
    memberRoles: Role[];
    assessmentRequestId: string;
    assessmentLayerId: string;
    layerCurrentStateId?: string | null;
    layerNextStateId?: string | null;
    requestCurrentStateId?: string | null;
    requestNextStateId?: string | null;
  }) {
    await this.actionLogBufferService.flushToActionLog(
      {
        assessmentRequestId: input.assessmentRequestId,
        assessmentLayerId: input.assessmentLayerId,
      },
      {
        userId: input.member.id,
        roleIds: input.memberRoles.map((role) => role.id),
        action: input.action,
        status: ActionLogStatusEnum.SUCCESS,
        assessmentRequestCurrentStateId: input.requestCurrentStateId ?? null,
        assessmentRequestNextStateId: input.requestNextStateId ?? null,
        assessmentLayerCurrentStateId: input.layerCurrentStateId ?? null,
        assessmentLayerNextStateId: input.layerNextStateId ?? null,
      },
    );
  }

  //------------------------------
  private async flushManyLayerStateActionLogs(input: {
    action: ActionEnum;
    member: Member;
    memberRoles: Role[];
    assessmentRequestId: string;
    layers: { id: string; stateId: string }[];
    nextLayerStateId?: string | null;
    requestCurrentStateId?: string | null;
    requestNextStateId?: string | null;
  }) {
    for (const layer of input.layers) {
      await this.flushLayerStateActionLog({
        ...
        assessmentLayerId: layer.id,
        layerCurrentStateId: layer.stateId,
        layerNextStateId: input.nextLayerStateId ?? null,
        requestCurrentStateId: input.requestCurrentStateId ?? null,
        requestNextStateId: input.requestNextStateId ?? null,
      });
    }
  }
}
