...
export class AssetVersionRepository extends AbstractRepository<AssetVersion> {
  ...
  async findGroupedByAssetTypeForDashboard(
    ...
    qb.leftJoinAndSelect('assetVersion.assetTypeVersion', 'assetTypeVersion')
      ...
      .select('assetType.id', 'assetTypeId')
  ...
  async findAllPaginationWithFilterQueryBuilder(
    ...
    if (query.supervisorEmployeeIds && query.supervisorEmployeeIds.length > 0) {
      qb.andWhere(
        `(assetVersion.accountableId IN (:...accountableIds) OR assetVersion.editorId IN (:...editorIds))`,
        {
      ...
      qb.andWhere(
        `(assetVersion.accountableId IN (:...assetUserScopeIds) OR assetVersion.editorId IN (:...assetUserScopeIds))`,
        {
      ...
      qb.andWhere(
        `(assetVersion.accountableId = :userId OR assetVersion.editorId = :userId)`,
        {
