...
describe('AssetController (e2e)', () => {
  ...
  //
  // sampa/api/v1/admin/assessment-type (GET)
  //
  it('sampa/api/v1/admin/assessment-type (GET) should return 401 when no token', async () => {
    await request(app.getHttpServer())
      .get('/sampa/api/v1/admin/assessment-type')
      .expect(401);
  });

  it('/sampa/api/v1/admin/assessment-type (GET) should return 400', async () => {
  ...
  it('/sampa/api/v1/admin/assessment-type (GET) should return 200', async () => {
    await connectionSource.getRepository(AssetType).save({ name: 'string' });

    mockedAxios.get.mockImplementation((url) => {
      if (url === 'mockedValue/idp/api/v1/admins/current-admin') {
        return Promise.resolve({
          data: {
            message: 'successful',
            data: userMock,
    ...
    const res = await request(app.getHttpServer())
      .get('/sampa/api/v1/admin/assessment-type?skip=0&take=10')
      .set('Authorization', `bearer ${token}`)
      .expect(200);
  ...
  //
  // sampa/api/v1/assessment-type (GET)
  //
  it('sampa/api/v1/assessment-type (GET) should return 401 when no token', async () => {
    await request(app.getHttpServer())
      .get('/sampa/api/v1/assessment-type')
      .expect(401);
  });

  it('/sampa/api/v1/assessment-type (GET) should return 400', async () => {
    mockedAxios.get.mockImplementation((url) => {
      if (url === 'mockedValue/idp/api/v1/users/current-user') {
        return Promise.resolve({
          data: {
  ...
  it('/sampa/api/v1/assessment-type (GET) should return 200', async () => {
    await connectionSource.getRepository(AssetType).save({ name: 'string' });
    ...
    const res = await request(app.getHttpServer())
      .get('/sampa/api/v1/assessment-type?skip=0&take=10')
      .set('Authorization', `bearer ${token}`)
      .expect(200);
