...
import { storageTemplates } from 'src/common/validations/excel-validation-template/storage-excel-template';
...
export class FileService {
  ...
  async validateAndProcessFile(
    ...
    } else if (fileTemplateType === FileTemplateTypeEnum.STORAGE_TEMPLATE) {
      validationTemplate = storageTemplates;
    }
  ...
  async processFile(
    ...
    } else if (query.template === FileTemplateTypeEnum.STORAGE_TEMPLATE) {
      result = await this.processStorageFile(
        user,
        userRoles,
        dataList,
        chunkSize,
    ...
    const processChunk = async (chunk: any[], startIndex: number) => {
      outerLoop: for (const [index, item] of chunk.entries()) {
        ...
        const existingVMWithName = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['NAME*']}%`),
            assetTypeId: existingVMType.id,
          },
        });
        ...
        if (ipArray.length > 0) {
          ...
          if (existingVMWithIP) {
            errors.push({
              ...
              message: `Row ${originalIndex + 1} - Warning : Virtual machine with IP ${ipArray[0]} and Name ${item['NAME*']} already exists`,
            });
            continue outerLoop;
          }
        }
        ...
        if (existingVMWithName) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Virtual machine with Name ${item['NAME*']} and IP ${ipArray[0]} already exists`,
          });
        }
        ...
        if (item['OS_TYPE_VERSION']) {
          ...
          if (
            ...
            errors = errors.concat(
              `Row ${originalIndex + 1} - Error : Related Asset Not Found OS With ExternalRefId ${`${item['OS_TYPE_VERSION']}`}`,
            );
        ...
        const networkArray = ipArray.map((ip) => ({
          IPAddress: ip,
        }));
  ...
  async processClientFile(
    ...
    const processChunk = async (chunk: any[], startIndex: number) => {
      outerLoop: for (const [index, item] of chunk.entries()) {
        ...
        const externalRefId = `${item['DEVICE_NAME**']}@${item['NETWORK_IP**']}`;
        ...
        const existingClientWithName = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['DEVICE_NAME**']}%`),
            assetTypeId: existingClientType.id,
          },
        });

        const existingClientWithIP = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['NETWORK_IP**']}%`),
            assetTypeId: existingClientType.id,
          },
        });
        ...
        if (existingClientWithIP) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Client with IP ${item['NETWORK_IP**']} and Name ${item['DEVICE_NAME**']} already exists`,
          });
          continue outerLoop;
        }
        ...
        if (existingClientWithName) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Client with Name ${item['DEVICE_NAME**']} and IP ${item['NETWORK_IP**']} already exists`,
          });
        }
        ...
        if (!existingClient && existingClientType) {
          ...
          const clientBody = {
            ...
            content: {
              ...
              Network: {
                IPAddress: item['NETWORK_IP**'],
                MACAddress: item['NETWORK_MAC*'],
                Zone: item['NETWORK_ZONE*'],
                Domain:
                  item['NETWORK_DOMAIN*'] === 'Iranet' ? 'Iranet' : 'Operation',
              },
              ConnectionStatus: 'Connected',
              User: [
                {
                  Username: item['USER_NAME*'],
                  IsAdmin: item['USER_IS_ADMIN*'] == 'Y' ? !isAdmin : isAdmin,
                },
              ],
              Hardware: {
  ...
  async processStorageFile(
    ...
    const [existingStorageType] = await Promise.all([
      this.assetTypeService.findOneByName('Storage'),
    ]);

    let errors: any[] = [];
    const savedStorageIds: string[] = [];
    ...
    const processChunk = async (chunk: any[], startIndex: number) => {
      outerLoop: for (const [index, item] of chunk.entries()) {
        ...
        ipArray.push(
          ...(item['NETWORK_OTHER_IPS'] ?? '')
            .split(/[;,]/)
            .map((ip: string) => ip.trim())
            .filter(Boolean),
        );

        const externalRefId = `${item['NAME**']}@${item['NETWORK_MANAGEMENT_IP**']}`;
        const existingStorage = await this.assetRepository.findOne({
          where: { externalRefId },
        });
        ...
        if (existingStorage) {
          errors.push({
            step: 'Asset Creation',
            message: `Row ${originalIndex + 1} - Warning : Storage with external reference Id : ${existingStorage.externalRefId} is already exists`,
          });
          continue outerLoop;
        }

        const existingStorageWithName = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['NAME**']}%`),
            assetTypeId: existingStorageType.id,
          },
        });

        const existingStorageWithIP = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['NETWORK_MANAGEMENT_IP**']}%`),
            assetTypeId: existingStorageType.id,
          },
        });

        if (existingStorageWithIP) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Storage with IP ${item['NETWORK_MANAGEMENT_IP**']} and Name ${item['NAME**']} already exists`,
          });
          continue outerLoop;
        }

        if (existingStorageWithName) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Storage with Name ${item['NAME**']} and IP ${item['NETWORK_MANAGEMENT_IP**']} already exists`,
          });
        }
        ...
        let createdStorage: CreatedAssetFromFileResponseDto;

        if (!existingStorage && existingStorageType) {
          const storageBody = {
            assetTypeVersionId: existingStorageType.latestVersion.id,
            name: item['NAME**'],
            externalRefId,
            accountableId: inquiryAccountableData.data.IdpUserId!,
            accountableUnitId: inquiryAccountableData.data.UnitId!,
            ...
            content: {
              ...
              StorageCapacity: {
                TotalTB: String(item['STORAGE_CAPACITY(GB)*'] / 1024),
              },
              SerialNumber: item['SERIAL_NO'],
              Network: {
                ManagementIP: item['NETWORK_MANAGEMENT_IP**'],
                ManagementMAC: item['NETWORK_MAC'],
                OtherNetworkAdaptors: ipArray.map((ip) => ({
                  IP: ip,
                })),
              },
              ...
              DataConfidentiality: 'NotRestricted',
            },
            locationId: existingLocation?.id,
          };

          createdStorage = await this.assetService.createFromFile(
            user,
            userRoles,
            storageBody,
          );

          if (this.isErrorResponse(createdStorage)) {
            createdStorage.errors.map((error) => {
              error.message = `Row ${originalIndex + 1} - Error : ${error.message}`;
            });

            errors = errors.concat(createdStorage.errors);
            continue;
          }

          if (this.hasAsset(createdStorage)) {
            savedStorageIds.push(createdStorage.asset.id);
          }
        }
      }
    };
  ...
  async processServerFile(
    ...
    const processChunk = async (chunk: any[], startIndex: number) => {
      outerLoop: for (const [index, item] of chunk.entries()) {
        ...
        let ipArray: string[] = [item['NETWORK_MANAGEMENT_IP**']];
        ipArray = [
          ...ipArray,
          ...(item['NETWORK_OTHER_IPS'] ?? '')
            .split(/[;,]/)
        ...
        const externalRefId = `${item['HOSTNAME**']}@${ipArray[0] || ''}`;
        ...
        const existingServerWithName = await this.assetRepository.findOne({
          where: {
            externalRefId: ILike(`%${item['HOSTNAME**']}%`),
            assetTypeId: existingServerType.id,
          },
        });
        ...
        if (existingServerWithIP) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Server with IP ${ipArray[0]} and Name ${item['HOSTNAME**']} already exists`,
          });
          continue outerLoop;
        }
        ...
        if (existingServerWithName) {
          errors.push({
            step: 'Duplicate Check',
            message: `Row ${originalIndex + 1} - Warning : Server with Name ${item['HOSTNAME**']} and IP ${ipArray[0]} already exists`,
          });
        }
        ...
        const hasOSTypeAndVersion = Boolean(item['OS_TYPE_VERSION*']);

        // if (!hasOSTypeAndVersion) {
        //   errors.push({
        //     step: 'Validation',
        //     message: `Row ${originalIndex + 1} - OS type/version must be provided`,
        //   });
        //   shouldContinueLoop = true;
        // }
        ...
        if (item['HYPERV_TYPE_VERSION']) {
          const existingHypervisor =
            await this.assetRepository.findOneCaseInsensitive(
              'externalRefId',
              item['HYPERV_TYPE_VERSION'],
        ...
        if (item['STORAGE_NAME'] && item['STORAGE_IP']) {
          const STORAGE_NAME_ARRAY = item['STORAGE_NAME'].split(';');
          const STORAGE_IP_ARRAY = item['STORAGE_IP'].split(';');
          for (let i = 0; i < STORAGE_NAME_ARRAY.length; i++) {
            const STORAGE_NAME = STORAGE_NAME_ARRAY[i];
            const STORAGE_IP = STORAGE_IP_ARRAY[i];
            const existingStorage =
              await this.assetRepository.findOneCaseInsensitive(
                'externalRefId',
                `${STORAGE_NAME}@${STORAGE_IP}`,
            ...
            if (existingStorage?.assetVersions?.[0]?.id) {
              ...
              errors = errors.concat(
                `Row ${originalIndex + 1} - Error : Related Asset Not Found Storage ${STORAGE_NAME}@${STORAGE_IP}`,
              );
              shouldContinueLoop = true;
            }
          }
        ...
        const serverBody = {
          ...
          content: {
            ...
            Network: {
              ...
              Zone: item['NETWORK_ZONE*'],
              ManagementMAC: item['NETWORK_MAC'],
              OtherNetworkAdaptors: (item['NETWORK_OTHER_IPS'] ?? '')
                .split(/[;,]/)
                .map((ip: string) => ip.trim())
                .filter(Boolean)
                .map((ip: string) => ({
            ...
            SerialNumber: item['SERIAL_NO'],
            Environment:
              item['NETWORK_ZONE*'] === 'Iranet' ? 'Iranet' : 'Operation',
            RAM: {
              AllocatedGB: String(item['RAM(GB)*']),
            },
            CPU: {
  ...
  async getFileExcelTemplate(query: ApplyFileQueryDto) {
    ...
    const templateMap: Record<FileTemplateTypeEnum, string> = {
      ...
      [FileTemplateTypeEnum.STORAGE_TEMPLATE]: 'storage',
    };
